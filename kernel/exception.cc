...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr)
{
  ...
  case SYSCALL_EXCEPTION: {
	...
	case SC_SEM_CREATE: {
		...
		int name_addr = g_machine->ReadIntRegister(4); //address of the string
		int name_size = GetLengthParam(name_addr);
		char name[name_size];
		GetStringParam(name_addr, name, name_size);

		int initValue = g_machine->ReadIntRegister(5); // initial value
		if (initValue < 0)
			g_syscall_error->SetMsg((char*)"", INVALID_COUNTER);
		Semaphore *sem = new Semaphore(name, initValue);

		int semId = g_object_ids->AddObject(sem);
	...
	case SC_LOCK_CREATE: {
		...
		int lockId = g_object_ids->AddObject(lock);
	...
	case SC_COND_CREATE: {
		...
		int condId = g_object_ids->AddObject(cond);
	...
	case SC_COND_DESTROY: {
		...
		int condId = g_machine->ReadIntRegister(4);
