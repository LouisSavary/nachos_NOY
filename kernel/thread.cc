...
int Thread::Start(Process *owner, int32_t func, int arg) {
  ...
  int stackptr = owner->addrspace->StackAllocate();
  this->InitThreadContext(func, stackptr, arg);

  //contexte simulateur
  int8_t *stack_sim = AllocBoundedArray(SIMULATORSTACKSIZE);
  this->InitSimulatorContext(stack_sim, SIMULATORSTACKSIZE);

  //on signale au processus l'augmentation du nombre de ses threads
  owner->numThreads++;

  //on ajoute le thread au systeme
  g_alive->Append(this);
